...
public class MainActivity extends Activity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == TAKE_PICTURE_REQUEST_B) {
            if (resultCode == RESULT_OK) {
                ...
                byte[] cameraData = extras.getByteArray(CamerActivity.EXTRA_CAMERA_DATA);
                if (cameraData != null) {
                    mCameraBitmap = BitmapFactory.decodeByteArray(cameraData, 0, cameraData.length);
                    mCameraBitmap=CamerActivity.rotateImage(mCameraBitmap,90);
    ...
    private void startImageCapture() {
        startActivityForResult(new Intent(MainActivity.this, CamerActivity.class), TAKE_PICTURE_REQUEST_B);
    }
